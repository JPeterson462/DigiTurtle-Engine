...
void main(void) {
	vec4 fogColor = vec4(vec3(0.9), 1.0);
	float near = 1.0;
	...
	fogFactor = clamp(fogFactor, 1.0 - SKY_FOG, 1.0);
	vec4 foreground = mix(fogColor, texVal, fogFactor);
	vec4 background = texture2D(skyTexture, pass_TextureCoord);
	out_Color = foreground + background * (1.0 - foreground.a);
	if (depth > far) {
		out_Color = background * (1.0 - SKY_FOG) + fogColor * SKY_FOG;
	}
}
